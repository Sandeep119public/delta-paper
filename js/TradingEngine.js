...
class TradingEngine {
  ...
  executeMarket({symbol,side,lots,leverage,reason='MARKET'}){
    const lotSize=Number(this.config.LOT_SIZES?.[symbol]);if(!(lotSize>0))throw Error('Unknown contract size');
    const mark=this.price(symbol);if(!(mark>0))throw Error(this.mode==='replay'?'Replay price unavailable':'Live price unavailable');
  ...
  close(symbol,reason='MARKET_CLOSE'){
    const pos=this.state.get().positions?.[symbol];if(!pos)throw Error('No open position');
    return this.executeMarket({symbol,side:-pos.dir,lots:pos.lots,leverage:pos.lev,reason});
  }
  onPrice(symbol,price){
    ...
    if(pos.tp&&((pos.dir===1&&price>=pos.tp)||(pos.dir===-1&&price<=pos.tp)))return this.close(symbol,'TAKE_PROFIT');
    if(pos.sl&&((pos.dir===1&&price<=pos.sl)||(pos.dir===-1&&price>=pos.sl)))return this.close(symbol,'STOP_LOSS');
    return null;
  }
}
