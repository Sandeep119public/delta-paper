/* TradingView-style chart integration with cached history and live Delta handoff. */
(function(global){
'use strict';
...
DeltaPaperApp.prototype._initChart=function(){
 ...
 this._chartData=global.chartDataService;this._chartRequest=0;this._loadCandles(this.selSym,this._tf);
...
DeltaPaperApp.prototype._loadCandles=async function(sym,tf){
 ...
 const end=Date.now(),start=end-this._tfSec*500000;
...
DeltaPaperApp.prototype._loadOlder=async function(){const svc=this._chartData||global.chartDataService;if(!svc||!this._historyCache.length)return;this._loadingOlder=true;const oldRange=this._tvChart.timeScale().getVisibleRange(),first=this._historyCache[0].time*1000,span=this._tfSec*500000;try{const older=cleanRows(await svc.getCandles(this.selSym,this._tf,Math.max(0,first-span),first-1));const map=new Map(this._historyCache.map(c=>[c.time,c]));older.forEach(c=>map.set(c.time,c));this._setChartData([...map.values()],oldRange);}catch(e){DELTA_LOGGER.warn('[Chart] older history failed',e);}finally{this._loadingOlder=false;}};
DeltaPaperApp.prototype._feedTick=function(price){
 if(!this._tvCandle||!(price>0))return;const now=Date.now(),bucket=Math.floor(now/1000/this._tfSec)*this._tfSec,m=this.market.getMarket(this.selSym),size=Number(m&&m.lastTradeSize),volume=Number.isFinite(size)&&size>0?size:1;let c=this._curCandle;
