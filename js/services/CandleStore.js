...
class CandleStore {
  ...
  intervalMs(interval){const m=String(interval).match(/^(\d+)([mhdw])$/i);if(!m)return 60000;const n=+m[1],u=m[2].toLowerCase();return n*(u==='m'?60000:u==='h'?3600000:u==='d'?86400000:604800000);}
  async latest(symbol,interval,limit=1000){const ms=this.intervalMs(interval),end=Date.now();return this.getRange(symbol,interval,end-ms*limit*2,end,limit);}
}
