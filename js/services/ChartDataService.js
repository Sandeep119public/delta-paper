 ...
 * IndexedDB first; Binance is used only to fill missing 1m history.
 * Higher intervals are aggregated locally from the 1m source of truth.
 */
class ChartDataService {
  constructor(config,candleStore){this.config=config;this.store=candleStore;this.baseInterval='1m';this.baseMs=60000;this.binanceBase=(config.BINANCE_KLINES_BASE||'https://api.binance.com/api/v3/klines').replace(/\/$/,'');}
  ...
  async fetchBinance(symbol,start,end,limit=1000){const u=`${this.binanceBase}?symbol=${encodeURIComponent(symbol)}&interval=1m&startTime=${Math.floor(start)}&endTime=${Math.floor(end)}&limit=${Math.min(1000,limit)}`;const r=await fetch(u);if(!r.ok)throw Error('Binance klines HTTP '+r.status);return r.json();}
  async ensureBase(symbol,start,end){const have=await this.store.getRange(symbol,'1m',start,end,100000),step=this.baseMs;if(have.length&&have[0].openTime<=start+step&&have[have.length-1].openTime>=end-step)return have;let cursor=Math.max(0,Math.floor(start/step)*step);while(cursor<end){const batch=await this.fetchBinance(symbol,cursor,Math.min(end,cursor+1000*step-1));if(!batch.length)break;await this.store.putMany(symbol,'1m',batch);const last=+batch[batch.length-1][0];if(last<cursor)break;cursor=last+step;if(batch.length<1000)break;}return this.store.getRange(symbol,'1m',start,end,100000);}
