...
await loadScript('./js/FinancialEngine.js'); await loadScript('./js/FinancialPatch.js'); await loadScript('./js/VisualizationPatch.js'); await loadScript('./js/BacktestPatch.js');
...
const orderEngine=new OrderEngine(config,marketService,eventBusInstance),fundingSimulator=new FundingSimulator(config,stateManager,marketService,eventBusInstance);
const riskMetrics=new RiskMetrics(config,stateManager),keyboardShortcuts=new KeyboardShortcuts(config,eventBusInstance),simulationEngine=new SimulationEngine(config,eventBusInstance);
const backtestEngine=new BacktestEngine(config,stateManager,eventBusInstance),monteCarloEngine=new MonteCarloEngine(eventBusInstance),app=new DeltaPaperApp(config,stateManager,validator,marketService);
window.app=app;window.orderEngine=orderEngine;window.fundingSimulator=fundingSimulator;window.riskMetrics=riskMetrics;window.keyboardShortcuts=keyboardShortcuts;window.simulationEngine=simulationEngine;window.backtestEngine=backtestEngine;window.monteCarloEngine=monteCarloEngine;window.eventBus=eventBusInstance;
